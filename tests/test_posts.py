...
def test_unauthorized_user_post_delete(client,test_user,test_user2,test_posts):
    res= client.delete(f"/posts/{test_posts[0].id}")
    assert res.status_code==401

def test_deleting_other_user_post(authorized_client,test_posts):
...
def test_update_post(authorized_client,test_posts):
    ...
    res= authorized_client.put(f"/posts/{test_posts[0].id}",json=update)
...
def test_update_other_user_post(authorized_client,test_posts):
    ...
    res=authorized_client.put(f"posts/{test_posts[3].id}",json=update)
